/**
 * Recovery Phrase Utility
 * 
 * Generates and validates recovery phrases for password reset functionality.
 * Uses the official BIP39 word list (2048 words) for maximum entropy.
 */

// Official BIP39 English word list (2048 words for 11 bits of entropy per word)
// 12 words = 132 bits of entropy
const BIP39_WORD_LIST = [
...
const STORAGE_KEY = "vault_recovery_hash";
const SALT_KEY = "vault_recovery_salt";
...
/**
 * Generate a random recovery phrase (12 words from BIP39 list)
 * Provides 132 bits of entropy (12 words × 11 bits per word)
 */
export function generateRecoveryPhrase(): string {
  const words: string[] = [];
  
  // Use 16 bytes (128 bits) of randomness
  // We need 11 bits per word, so we'll use a bit-extraction approach
  const randomBytes = new Uint8Array(17); // 136 bits, we use 132
  crypto.getRandomValues(randomBytes);
  
  // Convert bytes to bits and extract 11-bit indices
  let bitBuffer = 0;
  let bitsInBuffer = 0;
  let byteIndex = 0;
  
  for (let i = 0; i < 12; i++) {
    // Need 11 bits for each word index (2048 = 2^11)
    ...
    // Extract 11 bits
    bitsInBuffer -= 11;
    const wordIndex = (bitBuffer >> bitsInBuffer) & 0x7FF; // 0x7FF = 2047
    words.push(BIP39_WORD_LIST[wordIndex]);
  }
...
/**
 * Hash the recovery phrase for secure storage using PBKDF2
 */
async function hashRecoveryPhrase(phrase: string, salt: Uint8Array): Promise<string> {
  const encoder = new TextEncoder();
  // Normalize: lowercase, trim, collapse whitespace
  const normalizedPhrase = phrase.toLowerCase().trim().replace(/\s+/g, " ");
  const phraseData = encoder.encode(normalizedPhrase);
...
/**
 * Store the recovery phrase hash (call during vault setup)
 */
...
export async function verifyRecoveryPhrase(phrase: string): Promise<boolean> {
  ...
  const computedHash = await hashRecoveryPhrase(phrase, salt);
 ...
 */
export function hasRecoveryPhrase(): boolean {
  return localStorage.getItem(STORAGE_KEY) !== null;
}

/**
 * Clear the recovery phrase (for vault reset)
 */
...
/**
 * Validate that a phrase contains only valid BIP39 words
 */
export function validateRecoveryPhraseFormat(phrase: string): { valid: boolean; error?: string } {
